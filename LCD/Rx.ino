...
void OnDataRecv(const esp_now_recv_info_t * info, const uint8_t * incomingData, int len) {
    ...
    if (len == sizeof(TimeSyncPacket)) {
        ...
        Serial.printf(
            "%lld,%lld,%u,%u,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.2f,%.2f,%.2f,%.2f,%.6f,%.6f,%.2f,%.2f,%u,%u,%.2f\n",
            data.esp_elapsed_us,
            data.pc_us,
            data.utc_date,
            data.utc_time,
...
void setup() {
    ...
    Serial.println("ESP_us,PC_us,Date,Time,AccX,AccY,AccZ,GyrX,GyrY,GyrZ,MagX,MagY,MagZ,Temp,Roll,Pitch,Yaw,Lat,Lng,Speed,Alt,Sats,HDOP,Heading");
}
